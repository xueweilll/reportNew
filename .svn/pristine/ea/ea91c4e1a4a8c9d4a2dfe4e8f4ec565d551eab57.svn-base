	//新增窗口定义
	var $modal = $('#adddetails3_modal').modal({show: false});
	//手动定义排序规则
	  ...
	  window.actionEvents = {
	   ...
	   //显示模态窗口,
		function showModal(title, row) {
		    ...
		    $modal.modal('show');
			//新增、修改数据
		  $('#add_details3_form').bootstrapValidator({
			      fields: {
			    	  actualArrivalCustomerDate: {
					      validators: {
						      notEmpty: { 
						      	message: '客户收货日期不能为空'
					     	  },
			    	  ...
			    	  customerSignedReciptRequestReturnDate: {
					      validators: {
						      notEmpty: { 
						      	message: '客户签收日期不能为空'
					     	  },
			    	  ...
			    	  customerSignedReciptActualReturnDate: {
					      validators: {
						      notEmpty: { 
						      	message: '客户实际收货日期不能为空'
					     	  },
			                   date : {  
			                       format : 'YYYY-MM-DD',  
			                       message : '日期格式不正确'  
			  ...
			  //获取当前表单元素
				$.ajax({
			        ...
			        data: $("#add_details3_form").serialize(),
			        success: function(data){
			        	if(data){
			        		$table.bootstrapTable('refresh');
				        	$("input").val("");
				        	$modal.modal('hide');
				        	resetModel('add_details3_form');
		...
		$(document).ready(function() {
			...
			$('input[name$="Date"]').datepicker({
                ...
                $('#add_details3_form').data('bootstrapValidator')  
